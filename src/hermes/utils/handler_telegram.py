...
from config.settings import APPROVED_USERS
...
async def check_user_approval(event):
    if event.sender_id not in APPROVED_USERS:
        username = event.sender.username
        logger.info(f"Unauthorized access attempt from user {
                    event.sender_id} (username: @{username if username else 'N/A'})")
        await event.reply("This bot is currently in private mode.\n\nIf you believe you should have access, please contact @satriapamudji.")
        return False
    return True


async def check_callback_approval(event):
    if event.sender_id not in APPROVED_USERS:
        username = event.sender.username
        logger.info(f"Unauthorized callback query attempt from user {
                    event.sender_id} (username: @{username if username else 'N/A'})")
        await event.reply("This bot is currently in private mode.\n\nIf you believe you should have access, please contact @satriapamudji.", alert=True)
        return False
    return True
...
async def start_handler(event):
    if not await check_user_approval(event):
        return
...
async def research_handler(event):
    ...
    try:
        ...
        if user_state.get_state() == BotState.PROCESSING_RESEARCH:
            logger.info(f"Starting research process for user {
                        user_id} on topic: {topic}")
            ...
            logger.info(f"Research completed and file sent to user {
                        user_id}. File {pdf_filename} has been removed.")
            user_state.set_task_completed()
            await event.reply("Research task completed.")

    except Exception as e:
        logger.error(f"Error occurred during research process for user {
...
async def cancel_handler(event):
    ...
    if user_id in user_states:
        ...
        logger.info(
            f"User {user_id} attempted to cancel a task, but no active task was found.")
    raise events.StopPropagation
...
async def status_handler(event):
    ...
    if user_id in user_states:
        ...
        logger.info(
            f"User {user_id} checked status but no active task was found.")
    raise events.StopPropagation
...
async def courses_handler(event):
    ...
    try:
        ...
        logger.error(f"Error while fetching courses for user {
                     user_id}: {str(e)}", exc_info=True)
...
async def user_input_handler(event):
    if not await check_callback_approval(event):
        return
    ...
    if state == BotState.TASK_COMPLETED:
        logger.info(
            f"User {user_id} has completed their task, resetting state.")
        user_state.reset()
        return
...
async def course_detail_handler(event):
    ...
    try:
        ...
        logger.info(f"User {event.sender_id} requested details for course {
                    course_code} (ID: {course_id})")

        course_info = await get_canvas_data(f'courses/{course_id}')
        if course_info:
            reply = f"Course: {course_info.get('name', 'Unnamed Course')} ({
                course_code})\n\nSelect an option:"
            buttons = [
            ...
            logger.info(f"Displayed course details for {
                        course_code} to user {event.sender_id}")
        else:
            await event.edit("Failed to retrieve course information.",
                             buttons=[[Button.inline("Back to Courses", "back_to_courses")]])
            logger.warning(f"Failed to retrieve course information for course {
                           course_code} (ID: {course_id}) for user {event.sender_id}")
    except MessageNotModifiedError:
        pass
    except Exception as e:
        logger.error(f"Error while handling course details for user {
                     event.sender_id}: {str(e)}", exc_info=True)
...
async def courses_page_handler(event):
    ...
    try:
        ...
        if courses:
            ...
            logger.info(f"Displayed page {
                        page} of courses to user {event.sender_id}")
        else:
            await event.edit("Failed to retrieve courses. Please try again.",
                             buttons=[[Button.inline("Retry", "back_to_courses")]])
            logger.warning(f"Failed to retrieve courses for user {
                           event.sender_id} on page {page}.")
    except Exception as e:
        logger.error(f"Error while handling course page {page} for user {
                     event.sender_id}: {str(e)}", exc_info=True)
        await event.edit("An error occurred. Please try again.",
                         buttons=[[Button.inline("Retry", "back_to_courses")]])
    finally:
...
async def overview_handler(event):
    ...
    try:
        course_id, course_code = event.data.decode().split('_')[1:]
        logger.info(f"User {event.sender_id} requested overview for course {
                    course_code} (ID: {course_id})")

        modules = await get_canvas_data(f'courses/{course_id}/modules?include[]=items')
        if modules:
            module_list = [f"{i+1}. {module['name']}" for i,
                           module in enumerate(modules)]
            reply = f"Course Overview (Modules) for course {course_code}:\n\n" + "\n".join(
                module_list) + "\n\nSelect a module number to view its content:"

            module_buttons = [Button.inline(f"{i+1}", f"module_{course_id}_{module['id']}_{
                                            course_code}") for i, module in enumerate(modules)]
            module_buttons = [module_buttons[i:i+5]
                              for i in range(0, len(module_buttons), 5)]
            logger.info(f"Displayed overview for course {
                        course_code} to user {event.sender_id}")
        else:
            reply = "Failed to retrieve modules for this course. The course might not have any modules or there was an error."
            module_buttons = []
            logger.warning(f"Failed to retrieve modules for course {
                           course_code} (ID: {course_id}) for user {event.sender_id}.")

        buttons = module_buttons + [
            [Button.inline(f"Back to {course_code}", f"course_{
                           course_id}_{course_code}")],
            [Button.inline("Back to Courses", "back_to_courses")]
        ]
        await event.edit(reply, buttons=buttons)
    except MessageNotModifiedError:
        ...
        logger.error(f"Error while handling overview for course {course_code} (ID: {
                     course_id}) for user {event.sender_id}: {str(e)}", exc_info=True)
        await event.edit("An error occurred. Please try again.",
                         buttons=[[Button.inline("Back to Courses", "back_to_courses")]])
    finally:
        await event.answer()
...
async def overview_content_handler(event):
    ...
    try:
        ...
        logger.info(f"User {event.sender_id} requested content for module {
                    module_id} in course {course_code}")
            ...
            for num, (action, id_or_index) in button_map.items():
                if action == 'download':
                    item_buttons.append(Button.inline(
                        f"{num}", f"download_{course_id}_{id_or_index}"))
                else:
                    item_buttons.append(Button.inline(f"{num}", f"item_{course_id}_{
                                        module_id}_{id_or_index}_{course_code}"))
            item_buttons = [item_buttons[i:i+5]
                            for i in range(0, len(item_buttons), 5)]
        else:
            reply = f"Failed to retrieve content for this module or the module is empty."
            item_buttons = []
            logger.warning(f"Failed to retrieve content for module {module_id} in course {
                           course_code} for user {event.sender_id}.")

        buttons = item_buttons + [
            [Button.inline("Back to Overview", f"overview_{
                           course_id}_{course_code}")],
            [Button.inline("Back to Courses", "back_to_courses")]
        ]
        ...
        logger.error(f"Error while handling module content for module {module_id} in course {
                     course_code} for user {event.sender_id}: {str(e)}", exc_info=True)
        await event.edit("An error occurred. Please try again.",
                         buttons=[[Button.inline("Back to Courses", "back_to_courses")]])
    finally:
        await event.answer()
...
async def overview_content_detail_handler(event):
    ...
    try:
        course_id, module_id, item_index, course_code = event.data.decode().split('_')[
            1:]
        logger.info(f"User {event.sender_id} requested details for item {
                    item_index} in module {module_id} for course {course_code}")
        ...
        if module_items and 0 <= item_index < len(module_items):
            ...
            if item['type'] == 'Page':
                    ...
                    if page_data and 'body' in page_data:
                        ...
                        if body:
                            ...
                            file_links = re.findall(
                                r'title="([^"]+)"[^>]*href="[^"]*?/courses/\d+/files/(\d+)[^"]*"', body)

                            processed_file_ids = set()
                            for file_name, file_id in file_links:
                                if file_id not in processed_file_ids:
                                    decoded_file_name = html.unescape(
                                        file_name)
                                    reply += f"• {decoded_file_name}\n"
                                    buttons.append([Button.inline(
                                        f"Download {decoded_file_name}", f"download_{course_id}_{file_id}")])
                                    processed_file_ids.add(file_id)
                ...
                if file_id:
                    buttons.append(
                        [Button.inline(f"Download {item['title']}", f"download_{course_id}_{file_id}")])
            ...
            buttons = []
            logger.warning(f"Failed to retrieve details for item {item_index} in module {
                           module_id} for course {course_code} for user {event.sender_id}.")

        buttons.extend([
            [Button.inline("Back to Module", f"module_{
                           course_id}_{module_id}_{course_code}")],
            [Button.inline("Back to Overview", f"overview_{
                           course_id}_{course_code}")],
            [Button.inline("Back to Courses", "back_to_courses")]
        ])
        ...
        logger.error(f"Error while handling item details for item {item_index} in module {
                     module_id} for course {course_code} for user {event.sender_id}: {str(e)}", exc_info=True)
        await event.edit("An error occurred. Please try again.",
                         buttons=[[Button.inline("Back to Courses", "back_to_courses")]])
    finally:
        await event.answer()
...
async def folders_handler(event):
    ...
    course_id, course_code = event.data.decode().split('_')[1:]
    logger.info(f"User {event.sender_id} requested folders for course {
                course_code} (ID: {course_id})")
...
async def folder_content_handler(event):
    ...
    logger.info(f"User {event.sender_id} requested content for folder {
                folder_id} in course {course_code} (ID: {course_id})")
...
async def folder_page_handler(event):
    ...
    try:
        ...
        if len(data) >= 4:
            ...
            logger.info(f"User {event.sender_id} requested page {
                        page} of folder {folder_id} in course {course_code}")
            ...
            logger.warning(f"Invalid folder page data received from user {
                           event.sender_id}.")
    except ValueError as e:
        logger.error(f"ValueError while processing page number for user {
                     event.sender_id}: {str(e)}", exc_info=True)
        await event.answer("Error processing page number. Please try again.")
    except Exception as e:
        logger.error(f"Unexpected error during folder page handling for user {
                     event.sender_id}: {str(e)}", exc_info=True)
...
async def back_to_courses_handler(event):
    ...
    try:
        ...
        if courses:
            ...
            logger.info(f"Displayed back to courses for user {
                        event.sender_id}.")
        else:
            await event.edit("Failed to retrieve courses. Please try again.",
                             buttons=[[Button.inline("Retry", "back_to_courses")]])
            logger.warning(f"Failed to retrieve courses for user {
                           event.sender_id} on back to courses request.")
    except MessageNotModifiedError:
        pass
    except Exception as e:
        logger.error(f"Error during back to courses request for user {
                     event.sender_id}: {str(e)}", exc_info=True)
        await event.edit("An error occurred. Please try again.",
                         buttons=[[Button.inline("Retry", "back_to_courses")]])
    finally:
...
async def download_file_handler(event):
    ...
    logger.info(f"User {event.sender_id} requested download for file {
                file_id} in course {course_id}.")
    ...
    try:
        ...
        if file_info and 'url' in file_info:
            ...
            logger.info(f"Downloading file {file_name} from URL {
                        file_url} for user {event.sender_id}")
            ...
            if downloaded_file:
                ...
                logger.info(f"File {file_name} sent to user {
                            event.sender_id} and removed from server.")
            else:
                await status_message.edit(f"Failed to download file: {file_name}")
                logger.warning(f"Failed to download file {
                               file_name} for user {event.sender_id}.")
        else:
            await status_message.edit(f"Failed to retrieve file information for file ID: {file_id}")
            logger.warning(f"Failed to retrieve file information for file ID: {
                           file_id} for user {event.sender_id}.")
    except Exception as e:
        logger.error(f"Error during file download for file ID: {
                     file_id} for user {event.sender_id}: {str(e)}", exc_info=True)
